...
config/captain-github  the captain's GitHub login, which is what makes an assignment to them a wait rather than a claim in the tracker's frontier; LOCAL, gitignored; absent = every assignment reads as a claim; inherited by secondmate homes; see docs/configuration.md "Captain identity"
config/tracker-repos  maps a project to the GitHub repository whose Issues carry its project state, which is what makes a dispatch file its task ticket and a cleanup close it without firstmate remembering; LOCAL, gitignored; absent = that project has no tracker and nothing is filed, which never blocks work; inherited by secondmate homes; see docs/configuration.md "Tracker repositories"
config/tracker-withhold  records the task rows a tracker sync must never publish, with the reason, so a decision not to publish survives the next dispatch's sync instead of being undone by it; LOCAL, gitignored; absent = nothing is withheld; inherited by secondmate homes; see docs/configuration.md "Withheld rows"
...
state/               runtime records and signals; gitignored
  ...
  <id>.stopped       written by a verified `exit`: this endpoint is agent-free BY DESIGN, with its reason and the one incarnation it binds to; bin/fm-stopped-lib.sh owns the record (docs/agent-control.md)
  ...
  <id>.ci-watch <id>.ci-watch-fired  private build-verdict watch sidecar and its single-fire marker, written only by bin/fm-ci-check.sh; removed by teardown
  ...
  .live-test-versions  recorded installed versions of tools the nine fleet-relevant live tests depend on; bin/fm-live-test-versions.sh compares and updates; bootstrap prints LIVE_TEST_VERSIONS_CHANGED when a version differs
  ...
  .hash-* .count-* .stale-* .stale-since-* .stopped-* .paused-* .terminal-mtime-* .terminal-resurfaced-* .wedge-escalations-* .progress-* .progress-suppressed-* .busy-stall-surfaced-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
Drive a worker's lifecycle through `bin/fm-control.sh <task-id> interrupt|exit|relaunch|rebind`, which owns the per-runtime mechanics, verifies each action, and never tears down or discards anything ([`docs/agent-control.md`](docs/agent-control.md)).
...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green` after CI is green, while `direct-PR` reports `done: PR <url> checks complete` after the check set reaches a complete verdict.
...
2. For `stale:`, inspect the recorded endpoint and load `stuck-crewmate-recovery` for a stopped, exited, looping, confused, or unresponsive worker; a deep-inspection reason also requires current-state and validation-log inspection.
3. For `check:`, act on the named poll result, including build verdicts relayed to the waiting worker, merges, Relay events, and process-to-event source results.
