...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (HERDR_ENV=1 when the runtime firstmate itself is executing inside a herdr pane), then herdr; herdr is the only runtime session backend this fork supports (docs/herdr-backend.md), removed backends (tmux, zellij, orca, cmux) survive only in git history, and codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
...
state/               runtime records and signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .stopped-* .churn-since-* .paused-* .terminal-mtime-* .terminal-resurfaced-* .terminal-repeat-* .wedge-escalations-* .writing-* .progress-* .progress-suppressed-* .busy-stall-surfaced-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
A `use` array written in declared rung order skips `spendPriority` ranking and routes to its own spawn-time ladder gate instead: the agy array through `fm_agy_ladder_gate` strict exhaustion, and the opencode default through the free-first capped descent (`bin/fm-opencode-ladder-lib.sh`).
...
Verify a filled brief with `bin/fm-brief.sh <id> --check` before dispatch; `bin/fm-spawn.sh` and `bin/fm-promote.sh` refuse a widened brief through the same gate.
