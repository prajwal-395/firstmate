...
An unchanged pane is an absence, not evidence of a stopped worker, so no wedge escalation is raised on that absence alone: before the timer may fire, and again on every poll while it runs, the watcher measures the pane's own process subtree through `bin/fm-progress-lib.sh`, which owns that contract.
A `progressing` measurement restarts the wedge window instead of escalating - so a genuinely wedged worker is still caught within `FM_STALE_ESCALATE_SECS` of the moment its progress actually stops - and is itself bounded, re-surfacing once per `FM_PAUSE_RESURFACE_SECS` with the reading attached.
A `stalled` or unmeasurable reading escalates as before, with the reading carried in the wake reason so the demanded inspection starts from evidence.
...
That hour-long bound was also the only thing covering a worker stopped mid-turn by a provider session limit, which never fires its turn-end hook and so keeps classifying busy while sitting at an empty composer; a busy pane holding one unchanged frame is now measured on the same terms, and a positive `stalled` verdict sustained across `FM_PROGRESS_STALL_SPAN_SECS` surfaces it for inspection long before that bound.
...
The one exception is a crew whose declared wait is ARMED - a registered watcher check exists for that task, so an independent artifact will wake firstmate when the wait clears: that pause takes the bounded cadence immediately, without the dead-agent requirement, because it is substantiated by fleet state rather than by the worker's claim.
That parked-alive-and-wanted state is what a `direct-PR` worker enters after handing its build off, and a pause with nothing armed is unchanged - it still surfaces once, because an idle live pane may be sitting at a gate the worker cannot report.
...
Every submit core sizes its confirmation window to the line it submitted, a policy owned once by `bin/fm-composer-lib.sh`.
