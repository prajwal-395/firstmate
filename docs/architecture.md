...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh` into the herdr adapter, which uses native agent-state submit confirmation on idle baselines, a composer empty fallback when native stays idle, and a pre-Enter rendered-footer transition when that baseline is unavailable.
The retries-exhausted queued-Enter decision is owned by `fm_composer_queued_enter_verdict` in `bin/fm-composer-lib.sh`; herdr provides only its backend-specific busy signals.
Composer classification has one shared owner, `bin/fm-composer-lib.sh`: herdr contributes only a screen capture plus declarative styled, cursor, identity, and row capabilities, while the shared classifier owns every shape and the `empty`/`pending`/`pending-unproven`/`unknown` verdict.
...
Unsupported supervisor backends refuse at daemon startup; only herdr is supported.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, metadata-only cleanup identity validation, selector resolution, and operation dispatch; `bin/backends/herdr.sh` is the only adapter ([`docs/herdr-backend.md`](herdr-backend.md)).
...
Runtime auto-detection reads only `HERDR_ENV=1`: a runtime firstmate executing inside a herdr pane resolves herdr, and every other environment falls through to the herdr default.
Unknown backend names fail loudly, and the removed backends (`tmux`, `zellij`, `orca`, `cmux`) are refused with a message naming herdr; they survive only in git history, and the adapter indirection is deliberately kept so pulling one back stays cheap.
For compatibility, a task meta may omit `backend=`; every reader treats a missing `backend=` field as `herdr`.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: herdr has a verified classifier for secondmate recovery.
Herdr is selected explicitly, by `config/backend`, or by runtime auto-detection: Treehouse remains its worktree provider, [`herdr-backend.md`](herdr-backend.md) owns current setup, CI coverage, and safety limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#herdr) owns active empirical evidence.
...
`/updatefirstmate` fast-forwards the running firstmate repo and registered secondmate homes from the tracked upstream without touching project clones.
...
A home that is itself a leased linked worktree advances its detached HEAD, because such a home can never hold the default branch while the checkout it is leased from does; a standalone clone stranded on a detached HEAD is still refused, since there the detachment signals a checkout that may hold unique commits.
...
Fleet state lives in each task's Herdr session backend, no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
