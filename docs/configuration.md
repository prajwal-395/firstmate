...
FM_AGY_SHARED_ROOT=                     # machine-wide root for the agy ladder's account-scoped launch reservations; default $XDG_STATE_HOME/firstmate/agy-account, `off` forces per-home accounting
FM_AGY_INFLIGHT_LOCK_WAIT=5             # seconds a dispatch waits for that account's reservation lock before deciding without it
...
FM_PROGRESS_CPU_MIN_PCT=2          # sustained CPU, in percent of one core, that counts as work when the watcher measures a pane's process subtree instead of its rendered tail; a changed subtree membership is a second, independent positive signal
FM_PROGRESS_MIN_SPAN_SECS=180      # minimum span before a no-signal measurement may be called stalled on the wedge path; kept below FM_STALE_ESCALATE_SECS so a verdict exists when that timer fires
FM_PROGRESS_STALL_SPAN_SECS=600    # span of measured nothing before a BUSY pane holding one unchanged frame is surfaced on its own - the shape a worker stopped mid-turn by a provider session limit takes; longer than the wedge path because it raises a new alarm, and still six times sooner than FM_BUSY_TURN_MAX_SECS
