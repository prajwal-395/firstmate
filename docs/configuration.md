...
A `use` array is written in rung order, and the spawn-time ladder gates own which rung actually launches, so ranking a ladder by quota numbers here would override those gates rather than inform them: the agy array goes through `fm_agy_ladder_gate` rung order with a refusal escalated, and the opencode default starts at free, descends to Go on a free cap, then to Codex Plus when Go is exhausted (`bin/fm-opencode-ladder-lib.sh`, `bin/fm-opencode-retry.sh`). Go is exhausted when quota-axi reports known zero effective availability for `all_models`, or when the existing reactive Go cap evidence is current; unavailable or unknown quota data alone does not count. Codex Plus is exhausted only on the same known-zero quota reading. If all three are exhausted, the spawn is refused with all three tiers named. This affects new spawns only; running workers keep their provider unless separately relaunched.
