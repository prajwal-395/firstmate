...
A non-lock file at the status-presentation-lock path is malformed rather than held, so the drain quarantines it aside as evidence and reports an unsafe-acquire failure immediately instead of waiting out the deadline, and the next drain retries the skipped presentation.
