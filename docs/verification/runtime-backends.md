...
Verified 2026-09-14 on Claude Code 2.1.236, tmux backend, through `FM_CLAUDE_LIVE_E2E=1 bin/fm-test-run.sh tests/fm-claude-trust-live-e2e.test.sh`: a real scout spawn on a project carrying the vendor default (`hasClaudeMdExternalIncludesApproved===false` with the warning never shown) registered trust, reached its brief with no dialog, delivered its report, and tore down with the lease returned; a second spawn against a genuine decline (warning shown, approval refused) refused and left no window, no task record, no leased slot, and no store entry.
Two lab-only behaviors from that run are worth repeating nowhere else: a set-but-empty `CLAUDE_CONFIG_DIR` parks the worker on the machine-scoped Bypass Permissions consent screen (the guard therefore unsets rather than empties it), and the lab's own variables were found in the tmux server's global environment afterwards (the guard scrubs back exactly its own values).
