...
3. First cycle: arm with agy's tracked background tool, as its own call:

   `run_command` with a background wait threshold (`WaitMsBeforeAsync`) on:
...
10. Never bundle the arm onto another command.
    A shell `&`, a truncating pipe, or bundling is denied automatically by the PreToolUse seatbelt (`bin/fm-arm-pretool-check.sh`) whenever this project's agy hooks are trusted.

agy injects a message automatically into the conversation when the background task completes or sends a notification. You can also interact with running tasks using the `manage_task` tool (actions: `status`, `kill`, `send_input`) or use the `schedule` tool.
When you receive a background task message for the arm:
1. Run `bin/fm-wake-drain.sh` first.
2. Read the background task message for the command output and reason line (or use `manage_task` with action `status` if necessary).
...
The primary project Stop hook runs `bin/fm-turnend-guard-agy.sh` as a backstop, not the normal wake path.
