...
| `fm-tracker.sh`          | Own firstmate's GitHub Issues project layer: typed tickets, blocking edges, frontier, claims, answers, comments, validation, and the comment wake |
| `fm-tracker-lib.sh`      | Own the tracker's type vocabulary, blocking-edge format, captain identity, decision shape, issue-graph query, own-comment record, and refusal guards |
