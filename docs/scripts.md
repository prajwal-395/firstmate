...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, and Herdr-lab briefs, and `--check` one for dispatch readiness |
| `fm-brief-lib.sh`        | Single owner of a task brief's four required scope fields and the emptiness test both the scaffold's `--check` and the spawn gate use |
...
| `fm-agy-ladder-tick.sh`  | Run one agy ladder evaluation for a home and queue its outcome; the entry point an agy worker's own turn end drives |
...
| `fm-tracker.sh`          | Own firstmate's GitHub Issues project layer: typed tickets, blocking edges, frontier, claims, holds, withholds, answers, comments, validation, the comment wake, and the task tickets the dispatch and the cleanup file and close |
| `fm-tracker-lib.sh`      | Own the tracker's type vocabulary, blocking-edge format, captain identity, decision shape, issue-graph query, own-comment record, per-project tracker repositories, task-ticket shape, mirrored task-list rows, and refusal guards |
