...
194 of the 195 serial scripts carry measured hints, including the 28 that previously balanced on the default weight; the 5121 ms native-Windows focused runner measurement for `tests/fm-pi-windows-shell-invocation.test.sh` from 2026-09-06T21:02Z is retained because the portable CI shards gate-skip that script, and the `tests/fm-remote-secondmate-launch-verify.test.sh` hint is retained because that script records no timing artifact on portable runners.
`tests/fm-secondmate-watcher-quiet.test.sh`, added after the measurement window, still balances on the conservative default weight until several green runs supply a slowest-observed hint; the coverage guard tracks it through `serial_unhinted=`.
