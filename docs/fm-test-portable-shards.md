...
On green CI run [32302063973](https://github.com/prajwal-395/firstmate/actions/runs/32302063973), that remainder accumulated ~39 min of script time across 119 scripts.
...
The hints came from that run's `fm-test-timing-portable-serial` artifacts on 2026-08-19, where the lane ran 119 scripts in 2326832 ms of serial work.
A script with no hint gets the conservative `PORTABLE_SERIAL_DEFAULT_WEIGHT_MS` default.
Hints only affect balance: the coverage guard keeps the partition complete and disjoint whatever they say, so a stale hint costs a slower shard rather than lost coverage.

| Lane | Script count | Estimated duration |
|---|---:|---:|
| `portable-serial-1of4` | 30 | 588664 ms (~588.7 s) |
| `portable-serial-2of4` | 31 | 588665 ms (~588.7 s) |
| `portable-serial-3of4` | 30 | 588663 ms (~588.7 s) |
| `portable-serial-4of4` | 31 | 588663 ms (~588.7 s) |
| imbalance | | 2 ms |

The single longest script, `tests/fm-pr-check-security.test.sh` at 196273 ms, is the floor for any shard count.
...
| portable serial 1-4 | job `timeout-minutes: 20` | Each balanced shard is about ten minutes, leaving roughly 2x hang-tripwire margin. |
