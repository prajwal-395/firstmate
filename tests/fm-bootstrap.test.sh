...
# 'MISSING: gh-axi (install: ...)', 'PRESENTATION_UNAVAILABLE: lavish-axi ...',
# 'UPGRADE: <tool> (installed: ..., required: ..., upgrade: ...)', and
...
# config/backend; the dev shell's ambient HERDR_ENV inside a herdr pane must
# not leak into fm_backend_name and flip a case onto auto-detected herdr.
# Unset it once so the suite resolves the herdr default unless a case says
...
make_fake_toolchain() {
  ...
  fm_fake_exit0 "$fakebin" herdr node chrome-devtools-axi
  add_real_jq "$fakebin"
...
test_no_mistakes_min_version() {
  local label version expected_ver mode case_dir fakebin out upgrade n
  n=0
  while IFS='^' read -r label version expected_ver mode; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    ...
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      upgrade)
        upgrade="UPGRADE: no-mistakes (installed: $expected_ver, required: 1.46.0, upgrade: curl -fsSL https://raw.githubusercontent.com/kunchenguid/no-mistakes/main/docs/install.sh | sh)"
...
older no-mistakes patch reports an upgrade^no-mistakes version v1.45.4 (fake)^1.45.4^upgrade
unparseable no-mistakes version reports an upgrade^no-mistakes development build^unknown^upgrade
ROWS
  ...
  while IFS='^' read -r label version expected_ver mode; do
    ...
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      upgrade)
        upgrade="UPGRADE: gh-axi (installed: $expected_ver, required: 0.1.29, upgrade: npm install -g gh-axi && gh-axi setup hooks)"
        [ "$out" = "$upgrade" ] || fail "$label: expected '$upgrade', got: $out" ;;
    esac
  done <<'ROWS'
...
older gh-axi patch reports an upgrade^0.1.19^0.1.19^upgrade
much older gh-axi minor reports an upgrade^0.0.9^0.0.9^upgrade
unparseable gh-axi version reports an upgrade^gh-axi development build^unknown^upgrade
ROWS
...
test_tasks_axi_min_version() {
  local label version expected_ver mode case_dir fakebin out upgrade n archive_body multi_id
  n=0
  while IFS='^' read -r label version expected_ver mode; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    ...
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      upgrade)
        upgrade="UPGRADE: tasks-axi (installed: $expected_ver, required: 0.2.4, upgrade: npm install -g tasks-axi)"
        [ "$out" = "$upgrade" ] || fail "$label: expected '$upgrade', got: $out" ;;
    esac
  done <<'ROWS'
...
older tasks-axi with features reports an upgrade^0.1.1^0.1.1^upgrade
the patch just below the floor reports an upgrade^0.2.3^0.2.3^upgrade
unparseable tasks-axi version reports an upgrade^tasks-axi development build^unknown^upgrade
tasks-axi at floor without archive-body reports an upgrade^0.2.4:noarchive^0.2.4^upgrade
tasks-axi at floor without multi-id reports an upgrade^0.2.4:nomulti^0.2.4^upgrade
ROWS
...
# --version: below the floor produces UPGRADE, while at or above is silent.
  ...
  while IFS='^' read -r label version expected_ver mode; do
    ...
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      upgrade)
        upgrade="UPGRADE: quota-axi (installed: $expected_ver, required: 0.1.29, upgrade: npm install -g quota-axi)"
        [ "$out" = "$upgrade" ] || fail "$label: expected '$upgrade', got: $out" ;;
    esac
  done <<'ROWS'
...
the patch just below the floor reports an upgrade^0.1.28^0.1.28^upgrade
much older quota-axi minor reports an upgrade^0.0.9^0.0.9^upgrade
unparseable quota-axi version reports an upgrade^quota-axi development build^unknown^upgrade
ROWS
...
test_removed_backends_report_invalid_configuration() {
  local backend case_dir fakebin out
  # tmux, orca, zellij, and cmux were removed from this fork; a home that
  # still selects one must fail closed with an actionable diagnostic, never
  # silently fall back or demand the removed backend's tools.
  while IFS='^' read -r backend; do
    [ -n "$backend" ] || continue
    case_dir="$TMP_ROOT/$backend-selected"
    mkdir -p "$case_dir/home/config"
    printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
    ...
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
      FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
    assert_contains "$out" "BACKEND_INVALID: $backend (known: herdr)" \
      "backend=$backend should fail closed naming herdr, got: $out"
  done <<'ROWS'
tmux
orca
zellij
cmux
ROWS
  pass "bootstrap: removed backends fail closed with BACKEND_INVALID naming herdr"
}

test_herdr_does_not_require_a_removed_backend_cli() {
  local case_dir fakebin out
  # herdr is session-provider-only: it requires its own CLI, jq, and
  # treehouse, never a removed backend's CLI. With all genuine deps present,
  # bootstrap must be silent.
  case_dir="$TMP_ROOT/herdr-deps-present"
  mkdir -p "$case_dir/home/config"
  printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  [ -z "$out" ] || fail "backend=herdr with its own deps present should be silent, got: $out"
  pass "bootstrap: herdr requires its own CLI + jq + treehouse, never a removed backend's CLI"
}

test_herdr_gates_own_cli() {
  local case_dir fakebin out
  # With the herdr session CLI absent, bootstrap must fail closed on the
  # genuine dep.
  ...
  assert_contains "$out" "MISSING_MANUAL: herdr (instructions: https://herdr.dev)" \
    "backend=herdr must fail closed on its own missing session CLI"
  ...
  pass "bootstrap: herdr gates its own CLI with manual-install guidance"
}
...
test_unknown_backend_reports_invalid_configuration() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  assert_contains "$out" "BACKEND_INVALID: bogus (known: herdr)" \
...
test_herdr_requires_jq() {
  ...
  printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
  printf '%s\n' herdr > "$case_dir/home/config/backend"
  # Session CLI present, jq deliberately masked below.
  fakebin=$(make_fake_toolchain "$case_dir")
  rm -f "$fakebin/jq"
...
SH
  out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$bash_env" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  assert_contains "$out" "MISSING: jq" "backend=herdr must fail closed on missing jq"
  pass "bootstrap: herdr requires jq (its genuine dep)"
}

test_treehouse_lease_check_applies_to_herdr() {
  local case_dir fakebin out
  # A lease-less treehouse IS a problem for herdr, which relies on treehouse
  # for worktrees.
  ...
  pass "bootstrap: the treehouse lease check applies to herdr's worktree provider"
}
...
make_routine_bootstrap_fixture() {
  ...
  {
    printf 'window=fakesession:fakepane1\n'
    printf 'kind=secondmate\n'
    printf 'harness=codex\n'
    printf 'backend=herdr\n'
    printf 'endpoint_task_id=sm\n'
    printf 'herdr_session=fakesession\n'
    printf 'herdr_workspace_id=fakeworkspace1\n'
    printf 'herdr_tab_id=faketab1\n'
    printf 'herdr_pane_id=fakepane1\n'
    printf 'home=%s\n' "$sm"
  } > "$home/state/sm.meta"
  fakebin=$(make_fake_toolchain "$case_dir")
  add_real_jq "$fakebin"
  # A fake herdr CLI serving a live codex agent in fakepane1, so the
  # secondmate liveness sweep reads sm as alive (silent without verbose
  # facts) without touching a real backend.
  cat > "$fakebin/herdr" <<'SH'
#!/usr/bin/env bash
args="$*"
case "$args" in
  *'pane get fakepane1'*)
    printf '%s\n' '{"result":{"pane":{"pane_id":"fakepane1"}}}'
    ;;
  *'agent get fakepane1'*)
    printf '%s\n' '{"result":{"agent":{"agent_status":"working"}}}'
    ;;
  *'pane process-info'*)
    printf '%s\n' '{"result":{"type":"pane_process_info","process_info":{"pane_id":"fakepane1","shell_pid":424242,"foreground_processes":[{"pid":424243,"name":"codex","argv":["codex"],"cmdline":"codex"}]}}}'
    ;;
esac
exit 0
SH
...
SH
  ...
  assert_contains "$out" "UPGRADE: tasks-axi (installed:" "the unaided run did not probe tasks-axi"
  ...
  assert_not_contains "$out" "UPGRADE: tasks-axi" "the handed-in verdict was ignored"
  ...
  assert_contains "$out" "UPGRADE: tasks-axi (installed:" "a malformed handoff value was trusted"
...
legacy default agy order is named as legacy^{"default":[{"harness":"agy","model":"Claude Opus 4.6 (Thinking)","effort":"high"},{"harness":"agy","model":"Gemini 3.1 Pro (High)","effort":"high"},{"harness":"agy","model":"Gemini 3.7 Flash (High)","effort":"high"}]}^exact^CREW_DISPATCH: legacy config/crew-dispatch.json - the ladder order is read from the legacy default dispatch profiles (no agy_ladder field), so setting the order there also changes default dispatch - move the order to agy_ladder so the two set independently
split agy_ladder with its own default is silent^{"default":[{"harness":"opencode","model":"opencode/muse-spark-1.3-contributor-free","provider":"opencode"},{"harness":"opencode","model":"opencode-go/muse-spark-1.3-contributor","provider":"opencode-go"}],"agy_ladder":["Claude Opus 4.6 (Thinking)","Gemini 3.1 Pro (High)","Gemini 3.7 Flash (High)"]}^empty^
