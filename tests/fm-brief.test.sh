...
test_ship_project_memory_wording() {
  ...
  assert_grep "run the firstmate helper \`$ROOT/bin/fm-ensure-agents-md.sh .\` in the worktree" "$brief" \
    "project-memory contract lost the absolute helper command"
  assert_grep "follow that helper's self-governance contract" "$brief" \
    "project-memory contract no longer defers to the ensure helper"
  assert_no_grep "follow \`$ROOT/bin/fm-ensure-agents-md.sh\`'s self-governance contract" "$brief" \
    "project-memory contract repeats the bare home helper path instead of naming it once"
  pass "fm-brief.sh: ship project-memory wording carries the AGENTS.md authoring bar"
}

# The firstmate-repo failure: the generated ship brief named the home path on
# every status, inbox, and helper line while the workspace stayed abstract, so
# the primary checkout read as the place to work. The scaffold now names the
# verified worktree as the only editable workspace and firstmate's home as
# explicitly not the workspace, and names the home helper path once instead of
# twice. This pins the generated bytes, not the prose intent, with the home
# and helper roots collapsed to one directory exactly as in production, where
# firstmate runs from its primary checkout.
test_ship_workspace_vs_home_is_explicit() {
  local home id brief mode count
  home="$TMP_ROOT/workspace-vs-home"
  mkdir -p "$home/data"
  for mode in no-mistakes direct-PR local-only; do
    id="brief-workspace-$(printf '%s' "$mode" | tr '[:upper:]' '[:lower:]')"
    ...
    assert_grep "Verify isolation before anything else" "$brief" \
      "$mode brief lost the worktree-isolation assertion"
    assert_grep 'blocked: launched in primary checkout, not an isolated worktree' "$brief" \
      "$mode brief changed the isolation refusal"
    assert_grep "# Workspace vs firstmate's home" "$brief" \
      "$mode brief did not name the workspace-vs-home contract"
    # shellcheck disable=SC2016 # single quotes are deliberate: the backticks must stay literal
    assert_grep 'the `git rev-parse --show-toplevel` path' "$brief" \
      "$mode brief did not define the workspace as the verified worktree path"
    assert_grep "Firstmate's home is \`$home\`" "$brief" \
      "$mode brief did not name firstmate's home as a concrete path"
    assert_grep "it is NOT your workspace" "$brief" \
      "$mode brief did not mark the home as not the workspace"
    assert_grep "the firstmate-home paths below are outside it" "$brief" \
      "$mode brief rule 2 did not place the home paths outside the worktree"
    assert_grep "that file lives in firstmate's home, outside your worktree" "$brief" \
      "$mode brief did not label the status file as a home path"
    assert_grep "$home/state/$id.status" "$brief" \
      "$mode brief lost the actionable absolute status path"
    assert_grep "$home/state/$id.inbox" "$brief" \
      "$mode brief lost the actionable absolute inbox path"
    count=$(grep -c -F "$home/bin/fm-ensure-agents-md.sh" "$brief")
    [ "$count" = 1 ] \
      || fail "$mode brief must name the home helper path exactly once, found $count"
  done
  FM_HOME="$home" FM_ROOT_OVERRIDE="$home" \
    "$ROOT/bin/fm-brief.sh" brief-workspace-scout some-proj --scout >/dev/null 2>&1 \
    || fail "scout scaffold failed"
  assert_no_grep "# Workspace vs firstmate's home" "$home/data/brief-workspace-scout/brief.md" \
    "scout brief grew the ship-only workspace contract"
  pass "fm-brief.sh: ship briefs name the verified worktree as the workspace and the home as not"
}
