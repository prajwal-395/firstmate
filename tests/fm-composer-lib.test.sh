...
# --- ghost-strip SGR edge cases (rescued from the removed tmux-reader suite) --
# These pin the shared ANSI-aware owner (fm_composer_strip_ghost) directly:
# dim/faint runs drop, SGR `2` payload selectors in color sequences do not.
...
test_strip_ghost_keeps_colored_text_with_2_payloads() {
  local out
  # These pin that the truecolor/256-color `2` payload SELECTOR is not
  # mistaken for the SGR-2 dim attribute. The truecolor foregrounds use a
  # BRIGHT colour (grok's real-input RGB 224,222,244, luminance ~225), because
  # a DARK truecolor foreground is itself a ghost signal (grok's placeholder).
...
test_strip_ghost_drops_dark_truecolor_ghost() {
  ...
  # foreground (empirically 38;2;50;47;70 .. 38;2;110;106;134, luminance
  # ~51..110, verified live against grok 0.2.93). The shared owner drops it
  # while keeping the bright prompt glyph, so an idle grok composer never
  # reads as pending.
...
test_strip_ghost_keeps_muse_composer_colors() {
  local out glyph
  # muse 0.1.0-R708.1's real captured composer rows. Its prompt glyph `⟩` is
  # truecolor 38;2;90;160;255 (luminance ~149.9) and its typed text is
  # 38;2;204;211;219 (~209.8), so the glyph clears the 128 default by the
  # narrowest margin in the fleet. Both must survive stripping: dropping the
  # glyph would empty an idle composer's plain row, and dropping the typed
  # text would read a pending pane as empty and make it an injection target.
  ...
  # The submitted-prompt row carries a background colour too; an SGR 48
  # payload must not be luminance-tested as if it were the foreground.
