...
# DESCENT ONLY, BY CAPTAIN'S RULING: the reverse direction - climbing back up
# when a rung above resets (#40) - was deliberately dropped, so a worker that
# descends stays on the lower rung for the rest of its run. The section that
# used to prove the climb (old section 7) is gone with it.
#
...
#   7. (REMOVED with #40 - climbing back up was deliberately dropped. A worker
#      that descends stays on the lower rung for the rest of its run; there is
#      no recovery direction left to prove.)
#   8. (REMOVED with #40 - the climb's hysteresis went with the climb.)
...
# record_at: the same reading, stamped at an explicit instant, so an evaluation
# that must see evidence from two instants at once carries it forward with it;
# a wall-clock reading would age out underneath the test and it would pass for
# the wrong reason.
...
case "${1:-}" in
  ...
  # The readiness gate (agy_wait_for_working) needs a busy screen to confirm
  # the brief is running. The covered path below means no dialog renders, so
  # a steady busy row is the whole story - and it must carry no dialog text,
  # or the gate would wait for an answer instead.
  capture-pane) printf 'esc to cancel                                Gemini 3.1 Pro (High)\n'; exit 0 ;;
esac
exit 0
SH
  chmod +x "$fakebin/tmux"
  # Upstream's no-op treehouse stub (tests/lib.sh): the spawns under test use
  # real linked worktrees, never pool leases, so no `treehouse get` path is
  # consulted. The fork's fm_fake_treehouse differs only in answering `get`.
...
spawn_agy_worker() {  # <dir> <id>
  ...
  # The spawn refuses an unfilled scaffold (upstream's brief placeholder
  # guard), so the brief carries both subsections; the fork's legacy one-line
  # body does not pass on this tree.
...
EOF
  ...
  # The readiness gate counts a busy turn at once only on a covered path, and
  # coverage is read from the launching user's own store - so the spawn runs
  # under a scratch HOME whose store already trusts the worktree. That keeps
  # the suite hermetic and deterministic (see the spend-gate suite's spawn).
  mkdir -p "$dir/scratch-home/.gemini/antigravity-cli"
  wt_abs=$(cd -P -- "$wt" && pwd -P)
  printf '{"trustedWorkspaces":["%s"]}\n' "$wt_abs" > "$dir/scratch-home/.gemini/antigravity-cli/settings.json"
  env PATH="$fakebin:$PATH" FM_ROOT_OVERRIDE='' FM_HOME="$home" HOME="$dir/scratch-home" \
...
test_a_spent_ladder_escalates_exactly_once() {
  ...
  # No upwards half exists any more: the refusal says what it could not prove
  # downwards, and nothing else, because there is no climb left to report on.
...
test_the_two_spellings_of_one_model_are_not_a_disagreement() {
  ...
  # refused here forever - silently disabling the descent for a worker whose
  # record was never wrong.
  local state out fakebin outer_path=$PATH
