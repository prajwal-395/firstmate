...
res=$(fm_agy_quota_read "Claude Opus 4.6 (Thinking)" "$state" "$(date +%s)")
...
fm_agy_quota_observe "Claude Opus 4.6 (Thinking)  |  ctx:  5.0%  |  quota:  20.0%  (2h 0m)" "$state"
assert_eq "20.0 0" "$(fm_agy_quota_read "Claude Opus 4.6 (Thinking)" "$state" "$(date +%s)")"
...
Claude Opus 4.6 (Thinking) | ctx: 5.0% | quota: 20.0% (2h 0m)" "$state"
assert_eq "20.0 0" "$(fm_agy_quota_read "Claude Opus 4.6 (Thinking)" "$state" "$(date +%s)")"
...
fm_agy_quota_observe "Claude Opus 4.6 (Thinking) | ctx: 5% | quota: 95.0% (5h 0m)" "$state"
now=$(date +%s)
...
for window in 'forever' '0m' 'soon' ''; do
  ...
  if [ -n "$window" ]; then
    fm_agy_quota_observe "Claude Opus 4.6 (Thinking) | ctx: 5% | quota: 20.0% ($window)" "$state"
  else
    fm_agy_quota_observe "Claude Opus 4.6 (Thinking) | ctx: 5% | quota: 20.0%" "$state"
  fi
  now=$(date +%s)
...
for bad in '--' 'n/a' '' 'NaN'; do
  ...
  fm_agy_quota_observe "Claude Opus 4.6 (Thinking) | ctx: 5% | quota: $bad (2h 0m)" "$state"
  res=$(fm_agy_quota_read "Claude Opus 4.6 (Thinking)" "$state" "$(date +%s)")
...
if ! command -v jq >/dev/null 2>&1; then
  ...
  PATH="$fakebin:$PATH" fm_agy_quota_poll "$state" \
    || fail "the poll must record a reading from a well-formed quota answer"
  assert_eq "20.1 0" "$(fm_agy_quota_read "Claude Opus 4.6 (Thinking)" "$state" "$(date +%s)")"
  assert_eq "88.3 0" "$(fm_agy_quota_read "Gemini 3.1 Pro (High)" "$state" "$(date +%s)")"
  ...
  fm_agy_quota_observe "Claude Opus 4.6 (Thinking) | ctx: 5% | quota: 42.0% (2h 0m)" "$state"
  for body in '' 'not json at all' '{"status":"SUCCESS","command":{"name":"usage","data":{"groups":[]}}}'; do
    fakebin=$(fake_agy "$state" "$body")
    rc=0
    PATH="$fakebin:$PATH" fm_agy_quota_poll "$state" || rc=$?
    [ "$rc" -eq 1 ] || fail "an unusable quota answer must report no reading (rc=$rc)"
    assert_eq "42.0 0" "$(fm_agy_quota_read "Claude Opus 4.6 (Thinking)" "$state" "$(date +%s)")"
  done
