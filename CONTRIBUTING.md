...
bin/fm-test-run.sh --compare-selections   # size every selection smallest-first before picking one (runs nothing)
...
`--changed` is cheap for an ordinary change and expensive for a high-fan-out one, so it is not reliably narrower than a lane: it prints a NOTICE when it exceeds the smallest lane by more than the runner's configured multiple, and `--compare-selections` sizes every option at once when you want the numbers before you commit to a run.
