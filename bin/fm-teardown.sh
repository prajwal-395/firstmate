...
if [ -f "$META" ] && [ ! -L "$META" ]; then
  ...
  [ -n "$TEARDOWN_LOCK_BACKEND" ] || TEARDOWN_LOCK_BACKEND=herdr
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" "$sub_state/$child_id.progress" "$sub_state/$child_id.stopped" \
      "$sub_state/$child_id.pi-ext.ts" "$sub_state/$child_id.omp-ext.ts" \
      "$sub_state/$child_id.grok-turnend-token" "$sub_state/$child_id.kimi-turnend-token" \
      "$sub_state/$child_id.agy-turnend-token" "$sub_state/$child_id.agy-session" \
...
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.progress" "$STATE/$ID.stopped" \
  ...
  "$STATE/.$ID.branch-outcome-index" "$STATE/.agy-pin-$ID" \
  "$STATE/.opencode-pin-$ID"
