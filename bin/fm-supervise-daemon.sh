...
#                                   from $HERDR_PANE_ID under herdr - then
#                                   a non-resolving fallback). A herdr
#                                   "<session>:<pane-id>"
...
#                                   executing inside - $HERDR_ENV=1 selects
#                                   herdr - falling back to
#                                   herdr). Only herdr is supported as a
#                                   supervisor backend in this fork; the daemon
#                                   refuses loudly at startup rather than trying
#                                   primitives against a pane it cannot read.
...
# Supervisor injection dispatches through bin/fm-backend.sh's generic
# per-backend primitives (busy/composer detection + verify-retry submit).
...
# --- tunables ---------------------------------------------------------------
# Supervisor backends this daemon knows how to inject into today: herdr only.
# Selecting anything else refuses loudly at startup (see
# docs/herdr-backend.md and AGENTS.md section 4's
# harness-verification discipline).
...
# supervisor-pane busy guard dispatch through bin/fm-backend.sh.
...
# case statement here). <backend> defaults to herdr when omitted, so every
...
# A wedged primary used to get NO active signal - only the passive
# state/.subsuper-inject-wedged marker, which nothing surfaces until the next
# fleet action (the 2026-07-10 overnight incident: 20 escalations sat buffered
# for 8.5h on a claude-on-herdr primary). These helpers add a configurable
# active alert that does not depend on any pane or its backend status-line:
# an OS-level macOS notification, a herdr notification, or a captain-supplied
# command (push to a phone, etc.).
# Every channel is best-effort - a missing or failing channel logs and is
# skipped, never crashing the daemon loop - and the durable marker stays
# exactly as before.
#
...
# an ERROR, drops a durable marker firstmate/recovery can surface,
# and attempts a configurable backend-independent active alert
# (wedge_alarm_notify). Nothing is lost - the buffer and the
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  # Backend-independent active alert. This can reach the captain even when every
  # pane is unreadable - the gap the 2026-07-10 overnight
...
#     herdr's normal idle-baseline path means native agent-state observed a
#     real turn start.
...
fm_super_main() {
  ...
  # --- auto-discover the supervisor BACKEND (herdr) first --------------------
  # Priority: FM_SUPERVISOR_BACKEND override > $HERDR_ENV=1
  # (herdr) > herdr fallback. Resolved before the target below, since target
  ...
  # backend-specific call against it (AGENTS.md section 4
  # harness-verification discipline). This is the clear refusal the task calls
  # for, instead of a confusing resolution error.
  ...
  # Priority: FM_SUPERVISOR_TARGET override >
  # $HERDR_PANE_ID (herdr, composed into "<session>:<pane-id>") > fallback.
  ...
  if discovered=$(discover_supervisor_target); then
    ...
    echo "warn: could not auto-discover supervisor pane (no FM_SUPERVISOR_TARGET or HERDR_ENV/HERDR_PANE_ID); falling back to '$discovered' — verify this is firstmate's pane" >&2
  fi
  ...
  # Dispatches through bin/fm-backend.sh so the supervisor pane is checked via
  # the herdr adapter.
