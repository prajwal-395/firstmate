...
# The ownership and readiness waits are INDEPENDENT protocol constants shared
# by the caller and the worker (both source this file), deliberately not
# derived from a single freshness window: deriving each wait from the window
# below it meant a change to the window silently re-sized the waits above it.
# The values keep the margins a healthy start needs. A replacement cannot take
# ownership until a killed predecessor's surviving heartbeat ages out of the
    ...
    case "$mtime" in
      ...
      *)
        ...
        if [ "$age" -le 10 ]; then
          ...
          observed="${observed:+$observed; }the last heartbeat was ${age}s ago, past the 10s freshness window"
        fi
        ;;
    esac
  else
