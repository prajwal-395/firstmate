...
# herdr 0.8.2, steering a mid-turn worker: ~0.1s at 8 and 79 characters,
# 1.2-1.6s at 307, and 3.3-7.4s at 615. claude on the same backend accepted
# every one of those lines inside a tenth of a second, which is why a constant
# window looked correct for years and then reported a landed steer as
# unsubmitted the moment a real firstmate decision message (a few hundred
# characters) went to agy.
#
