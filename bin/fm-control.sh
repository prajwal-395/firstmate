...
#              endpoint rather than the retired one.
...
#   - `exit` and `relaunch` require the herdr backend's recovery-grade
#     agent-state classifier, because without one the "the agent stopped"
...
do_exit() {
  ...
  case "$state" in
    ...
    missing)
      ...
      # called standalone (exit verb), the verb maps this back to the refusal
      # below - there is nothing to stop.
