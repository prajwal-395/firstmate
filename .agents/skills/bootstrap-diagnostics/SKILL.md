...
- `UPGRADE: <tool> (installed: <version>, required: <min>, upgrade: <command>)` - the tool is installed but below its version floor.
  Report both versions to the captain with the printed upgrade command, wait for consent (one approval may cover the list), then run the printed upgrade command.
  The floor is not negotiable: never argue it down to whatever the home happens to have installed.
  For `no-mistakes`, this means an installed version older than 1.46.0, because this repo's PR gate requires structured pipeline attestation that older builds do not write.
  For essential axi-family tools - `gh-axi`, `tasks-axi`, `quota-axi` - [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh) owns the floor policy.
