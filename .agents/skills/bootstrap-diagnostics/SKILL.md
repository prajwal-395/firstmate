...
- `UPGRADE: <tool> (installed: <version>, required: <min>, upgrade: <command>)` - the tool is installed but its version is below the required floor.
  Report both versions to the captain with the printed upgrade command and a one-line reason the floor rose (typically an upstream sync that raised the minimum).
  Wait for consent, then run the printed upgrade command.
  The floor is not negotiable: never argue it down to whatever happens to be installed.
  For `no-mistakes`, this means the installed version is older than the minimum crewmate-validation-brief version.
  For any axi-family tool - `gh-axi`, `lavish-axi`, `tasks-axi`, `quota-axi` - [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh) owns the floor policy.
  For `tasks-axi`, this additionally covers an installed build that fails the separate feature probe (`bin/fm-tasks-axi-lib.sh` owns the definition); `config/backlog-backend=manual` only suppresses the verbose `BOOTSTRAP_INFO: tasks-axi available` fact, not this upgrade report.
  For `quota-axi`, bootstrap requires it because firstmate reads its current output directly before resolving every crew-dispatch profile array; without a compatible version, report the upgrade requirement and do not choose around an unexamined candidate.
...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - <reason>` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch, report the actionable error, and require correction of the malformed schema, unverified harness name, or invalid harness/effort pair rather than falling back around it or selecting a bad profile.
  Two reasons name the agy LADDER rather than a profile - a model the ladder cannot rank, or one listed more than once across the two spellings agy accepts.
  Those do not stop dispatch: agy work keeps running on the built-in rung order, which is exactly the problem, because the captain's configured order is the one being ignored.
  Report which model, and that the enforced order is not the configured one, so the captain can correct the config rather than discovering the wrong order later from a spend.
...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the session-start liveness sweep could not guarantee that the registered secondmate is running a real agent process.
  ...
  Two of those reasons mean the agent is still there and the sweep refused on purpose rather than failing to reach it.
  A recorded endpoint identifier that no longer resolves while a live endpoint still carries the secondmate's identity is a stale record, not an absent agent: correct it with `bin/fm-control.sh <id> rebind` and then read its real state, never by respawning.
  A stopped agent is suspended, not gone: resume it in its own endpoint, because relaunching over it abandons a resumable worker.
