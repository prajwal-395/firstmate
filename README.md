...
- **Restart-proof** - all state lives on disk and in the Herdr session backend; kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
...
- The CLI and dependencies for the Herdr runtime backend.
...
The Herdr setup guide is linked in [Documentation](#documentation) below.
